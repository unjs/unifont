...
If your API can deliver a font file instanced to the axis values in `options.variableAxis`, request it and list the axes the file you return carries:

```ts
async resolveFont(family, options) {
  options.variableAxis // { CASL: ['1'], wdth: [['75', '100']] }
  const publishedAxes = new Set(getAxes(family))
  const applied = Object.keys(options.variableAxis ?? {}).filter(tag => publishedAxes.has(tag))

  return {
    fonts: await fetchInstancedFaces(family, options),
    appliedVariableAxis: applied,
  }
}
```

`options.variableAxis` is normalised before it reaches you: a single value is a string, a range is a `[min, max]` pair of strings. List an axis only when the file itself is instanced or limited to it, which for most catalogues varies from family to family. Don't list an axis you applied as a `font-variation-settings` descriptor. `unifont` turns the list into the report the caller reads, and writes no descriptor of its own for a family you resolved, since you know which axes the file carries.
