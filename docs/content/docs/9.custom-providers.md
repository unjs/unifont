...
If your API can deliver a font file instanced to the axis values in `options.variableAxis`, request it and report the values the file you return carries:

```ts
async resolveFont(family, options) {
  options.variableAxis // { CASL: ['1'], wdth: [['75', '100']] }
  const applied = pickPublishedAxes(family, options.variableAxis)

  return {
    fonts: await fetchInstancedFaces(family, applied),
    appliedVariableAxis: applied, // { CASL: ['1'] }
  }
}
```

`options.variableAxis` is normalised before it reaches you: a single value is a string, a range is a `[min, max]` pair of strings. Report the values the file really carries, which may be narrower than the request if you clamp to what a family publishes, and leave out any axis you could not apply. Don't report an axis you applied as a `font-variation-settings` descriptor. `unifont` turns this into the report the caller reads, and writes no descriptor of its own for a family you resolved, since you know which axes the file carries.

Omit `appliedVariableAxis` when you serve whole variable files. `unifont` then expresses single-value axes as `font-variation-settings` on the faces you return. See [variable axes](/docs/resolving#variable-axes).
...
import type { ProviderResolveFontOptions } from 'unifont'

export const foundry = defineFontProvider('foundry', async (options: FoundryOptions, ctx) => ({
  async resolveFont(family, options: ProviderResolveFontOptions<FoundryFamilyOptions>) {
...
Providers start lazily, the first time a request reaches them. If a provider throws while starting up, it's skipped and the rest carry on, unless the user set `throwOnError` and no other provider could serve the family.
