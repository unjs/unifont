...
Providers cache two things behind the same interface. There's the family index built when a provider starts: `listFonts` data, large, and it hardly ever changes. And there are per-family results, keyed by the options you passed: small, but a lot of them. Both go through the storage you provide, so one driver covers it.

Keep the instance around too, as the example above does. Starting a provider means fetching the whole family index for some of them, and that only happens once per instance. Creating one per request throws that work away.
