...
properties?.axes      // [{ tag: 'opsz', min: 6, max: 72, defaultValue: 14 }, …]
...
`fallbacks` holds the generic family the provider suggests (`serif`, `sans-serif`). (That's what you need if you're generating metric-adjusted fallbacks; see [Reducing layout shift](/docs/layout-shift).)
...
If a provider fails, `unifont` logs it and carries on with the next provider in the list, so one dead API doesn't break your build. A provider is only started the first time a request reaches it, so an outage at a provider that earlier ones already serve costs nothing. Pass `throwOnError` if you'd rather it stopped:

```ts
const unifont = await createUnifont([providers.google()], { throwOnError: true })
```

Even then, an error is only raised if no provider could serve the family. If one provider fails and a later one succeeds, the failure is a warning. When it does throw with more than one failure, you get an `AggregateError` whose `errors` carry the individual causes.
