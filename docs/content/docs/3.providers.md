...
If the characters you name span a whole curated subset, that subset's file will be served as is, since Google optimises those better than the files it builds on demand. Every other character you name, including any outside the subsets you asked for, comes back in one face built for exactly those characters.
...
The family name has to match what the package's own `@font-face` rules declare, which is not always the name you would search a CDN for. The [catalogue](/fonts?provider=npm) lists families this provider can resolve that the font CDNs do not carry, each with the exact call it needs.
