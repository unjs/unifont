// Test for npm provider functionality
import { describe, expect, it } from 'vitest'
import { createUnifont, providers } from '../../src'

// Before running this test, make sure the required font packages are installed
// pnpm add @fontsource/roboto -D

describe('npm provider integration test', () => {
  it('should resolve fonts from local npm packages', async () => {
    // Create a Unifont instance using only npm provider
    const unifont = await createUnifont([
      providers.npm({
        // Explicitly specify packages to check
    ...
    if (result.fonts.length > 0) {
      ...
      // Verify metadata
      if (font.meta) {
        expect(font.meta.source).toBe('npm')
  ...
  it('should prefer local fonts when preferLocal is true', async () => {
    ...
    const unifontWithPreferLocal = await createUnifont(
      ...
      { preferLocal: true }, // Enable preferLocal option
    )
    ...
    if (
      ...
      expect(resultWithPreferLocal.fonts[0].meta.source).toBe('npm')
    }

    // Create a Unifont instance using both providers but without enabling preferLocal
    const unifontWithoutPreferLocal = await createUnifont(
      [
        // Google provider first, npm provider second
        providers.google(),
        providers.npm({
          packages: ['@fontsource/roboto'],
        }),
      ],
      { preferLocal: false }, // Don't enable preferLocal option
    )

    // Resolve Roboto font
    const resultWithoutPreferLocal
      = await unifontWithoutPreferLocal.resolveFont('Roboto')
    expect(resultWithoutPreferLocal.provider).toBe('google')
  })
})
